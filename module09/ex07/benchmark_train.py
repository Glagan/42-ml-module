...
for i in range(0, max_polynomial):
    ...
    model = MyRidge(thetas[i] + 0, alpha=alpha, max_iter=max_iter)
    # Normalize dataset after polynomials
    x = add_polynomial_features(xTrain, i + 1)
    # print("thetas", thetas[i].shape)
    # print("x", x.shape)
    norm_mean, norm_std = x.mean(axis=0), x.std(axis=0)
    x = (x - norm_mean) / norm_std
    fdfdfd = model.fit_(x, yTrain, check_loss=True)
    _, loss = fdfdfd
...
np.savez("models.npz",
         np.array([alpha, max_iter, 4]),
         thetas[3],  # The fourth model is the best
         *[model.thetas for model in models[:4]])
