...
class MyLinearRegression:
    def __init__(self, thetas: np.ndarray, alpha: float = 0.0005, max_iter: int = 42000) -> None:
        if not isinstance(thetas, np.ndarray) or len(thetas.shape) != 2 or thetas.shape[1] != 1:
            raise ValueError(
                'thetas should be an array of at size (n,1) with n > 1')
    ...
    def gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if len(y.shape) != 2 or y.shape[0] != x.shape[0] or y.shape[1] != 1:
            return None
        if x.shape[1] != self.thetas.shape[0] and x.shape[1] + 1 != self.thetas.shape[0]:
            return None
        m1 = 1 / (2 * x.shape[0])
    ...
    def fit_(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.max_iter):
            self.thetas = self.thetas - (self.alpha * self.gradient(x_int, y))
        return self.thetas
